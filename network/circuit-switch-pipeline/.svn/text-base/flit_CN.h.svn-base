...
class flit_CN {
public:
	flit_CN(int id, int vc, int vnet, int size, MsgPtr msg_ptr, int srcNetIntfID, int destID);
	flit_CN(int vc, bool is_free_signal);
	...
	{
        m_stage_period[m_stage.first] += g_eventQueue_ptr->getTime() + 1 - m_stage.second;
    ...
    inline int get_delay_I()
    {
        return m_stage_period[I_];
    }
    inline int get_delay_VA()
    {
        return m_stage_period[VA_];
    }
    inline int get_delay_SA()
    {
        return m_stage_period[SA_];
    }
    inline int get_delay_ST()
    {
        return m_stage_period[ST_];
    }
    inline int get_delay_LT()
    {
        return m_stage_period[LT_];
    }
...
private:
    ...
    map<flit_stage, int> m_stage_period;

};

inline extern bool node_less_then_eq(flit_CN* n1, flit_CN* n2);

inline extern
bool node_less_then_eq(flit_CN* n1, flit_CN* n2)
{
...
ostream& operator<<(ostream& out, const flit_CN& obj);
