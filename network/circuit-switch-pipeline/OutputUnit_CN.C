...
OutputUnit_CN::OutputUnit_CN(int id, Router_CN *router, PortType portType)
{
	m_id = id;
	m_router = router;
//	m_num_vcs = m_router->get_num_vcs();
//	m_out_buffer = new flitBuffer_CN();
    m_portType = portType;
    ...
    {
        m_out_buffer.setSize(1);
        for(int j = 0; j < 1; j++)
        {
            m_out_buffer[j] = new flitBuffer_CN();
        }
    }
...
OutputUnit_CN::~OutputUnit_CN()
{
    for(int i = 0 ; i < m_out_buffer.size(); i++)
    {
        delete m_out_buffer[i];
    }
	//delete m_out_buffer;
//	m_outvc_state.deletePointers();
}
/*
void OutputUnit_CN::decrement_credit(int out_vc)
{
...
Vector<flitBuffer_CN*> OutputUnit_CN::getOutQueue()
{
	return m_out_buffer;
}	

void OutputUnit_CN::set_out_link(NetworkLink_CN *link)
{
	m_out_link = link;
}

//void OutputUnit_CN::set_credit_link(CreditLink_d *credit_link)
//{
//	m_credit_link = credit_link;
//}
/*
void OutputUnit_CN::update_vc(int vc, int in_port, int in_vc)
{
...
*/
void OutputUnit_CN::printConfig(ostream& out)
{
	out << endl;
