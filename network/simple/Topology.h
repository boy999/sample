...
public:
  // Constructors
#ifdef CS_NOC
  Topology(Network* network_ptr, int number_of_nodes, bool setupNet, TopoType topoType);
#else
  ...
  int getLatency(SwitchID src, SwitchID dst);

private:
  // Private Methods
#ifdef CS_NOC
  void init(bool setupNet, TopoType topoType);
#else
  ...
  void make2DTorus();
  void makePtToPt();
  void makeFileSpecified(TopoType);
