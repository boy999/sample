...
#endif
#ifdef SIM_MEMORY
#include "SimicsMemoryInterface.h"
#endif
#include <string>
...
static void extend_shortest_path_latency(Matrix edge_weights, Matrix& path_distances, Matrix& inter_switches, Matrix& max_weight_edge, Matrix& min_weight_edge);
static void shortest_path_latency(Matrix& edge_weights, Matrix& path_distances, Matrix& inter_switches, Matrix& max_weight_edge, Matrix& min_weight_edge);

static bool link_is_shortest_path_to_node_latency(SwitchID src, SwitchID next, SwitchID final, const Matrix& dist, const Matrix& max_weight_edge, const Matrix& min_weight_edge);
static NetDest shortest_path_to_node_latency(SwitchID src, SwitchID next, const Matrix& dist, const Matrix& max_weight_edge, const Matrix& min_weight_edge     );

#ifdef CS_NOC
Topology::Topology(Network* network_ptr, int number_of_nodes, bool setupNet=false, TopoType topoType=TopoType_Default)
#else
...
#endif
{
  ...
  m_unidirectional_links = false;
#ifdef CS_NOC
  init(setupNet, topoType);
#else
...
#ifdef CS_NOC
void Topology::init(bool setupNet, TopoType topoType)
#else
void Topology::init()
#endif
{ 
...
void Topology::makeFileSpecified(TopoType topoType)
{
  ...
  else if (topoType == TopoType_Butfly)
  {
     filename = filename+g_CACHE_DESIGN  
              +"_Procs-"+int_to_string(RubyConfig::numberOfProcessors())
              +"_ProcsPerChip-"+int_to_string(RubyConfig::numberOfProcsPerChip())
  ...
  else if (topoType == TopoType_Clos)
  {
     filename = filename+g_CACHE_DESIGN  
              +"_Procs-"+int_to_string(RubyConfig::numberOfProcessors())
              +"_ProcsPerChip-"+int_to_string(RubyConfig::numberOfProcsPerChip())
  ...
  else if (topoType == TopoType_Custom)
  {
     filename = filename+g_CACHE_DESIGN  
              +"_Procs-"+int_to_string(RubyConfig::numberOfProcessors())
              +"_ProcsPerChip-"+int_to_string(RubyConfig::numberOfProcsPerChip())
  ...
  while (!networkFile.eof()) {
      ...
      while (varStr != "") {
          ...
          } else if (label == "unidirectional")
          {
              m_unidirectional_links = true; 
#ifdef SIM_NET_PORTS
...
#define USE_LINK_LATENCY_FOR_SHORTEST_PATH 1
void Topology::createLinks(bool isReconfiguration)
{
    ...
    Matrix max_weight_edge;
    Matrix min_weight_edge;
    Matrix path_distances;

    int num_switches = max_switch_id+1;
    topology_weights.setSize(num_switches);
    topology_latency.setSize(num_switches);
    max_weight_edge.setSize(num_switches);
    min_weight_edge.setSize(num_switches);
    path_distances.setSize(num_switches);
    ...
    for(int i=0; i<topology_weights.size(); i++) {
        ...
        max_weight_edge[i].setSize(num_switches);
        min_weight_edge[i].setSize(num_switches);
        path_distances[i].setSize(num_switches);
        ...
        for(int j=0; j<topology_weights[i].size(); j++) {
            ...
            min_weight_edge[i][j]=INFINITE_LATENCY;
            max_weight_edge[i][j]=-1;
            path_distances[i][j] = INFINITE_LATENCY;
        }
    }
    ...
    for (int i=0; i<m_links_src_vector.size(); i++) {
        ...
        //if one of the endpoints is a machine node, treat differently
        if(m_links_src_vector[i] < 2*m_nodes || m_links_dest_vector[i] < 2*m_nodes)
        {
            //a flit should always be able to exit to a machine node, hence min_weight_edge of
            //router->machine link is high
            min_weight_edge[m_links_src_vector[i]][m_links_dest_vector[i]] = INFINITE_LATENCY;
            //a flit should always be able to enter the NoC from a machine node, hence max_weight_edge of
            //machine->router link is low
            max_weight_edge[m_links_src_vector[i]][m_links_dest_vector[i]] = -1;
        }
        else
        {
            min_weight_edge[m_links_src_vector[i]][m_links_dest_vector[i]] = m_links_weight_vector[i];
            max_weight_edge[m_links_src_vector[i]][m_links_dest_vector[i]] = m_links_weight_vector[i];
        }

        path_distances[m_links_src_vector[i]][m_links_dest_vector[i]] = m_links_latency_vector[i];
...
#ifndef USE_LINK_LATENCY_FOR_SHORTEST_PATH
...
#else
    extend_shortest_path_latency(topology_weights, path_distances, m_component_inter_switches,
            max_weight_edge, min_weight_edge);
    for(int i=0;i<path_distances.size();++i)
        for(int j=0;j<path_distances[i].size();++j)
            m_component_latencies[i][j] = path_distances[i][j];
    for(int i=0; i<topology_weights.size(); i++)
    {
        for(int j=0; j<topology_weights[i].size(); j++)
        {
            ...
            if (weight > 0 && weight != INFINITE_LATENCY)
            {
                NetDest destination_set = shortest_path_to_node_latency(i, j, path_distances, max_weight_edge, min_weight_edge);
                assert(latency != -1);
                makeLink(i, j, destination_set, latency, weight, bw_multiplier, isReconfiguration);
            }
        }
...
int Topology::getLatency(SwitchID src, SwitchID dst)
{
	assert(src < m_component_inter_switches.size());
	assert(dst < m_component_inter_switches[src].size());
	//assert(src < m_nodes && dst >= m_nodes && dst <= 2*m_nodes);
	return m_component_latencies[src][dst];
}
/**************************************************************************/
...
static void extend_shortest_path_latency(Matrix edge_weights, Matrix& path_distances, Matrix& inter_switches,
        Matrix& max_weight_edge, Matrix& min_weight_edge)
{
    bool change = true;
    int nodes = edge_weights.size();
    ofstream fptr;
    fptr.open("debug_routing.log", ios::out);
    while (change)
    {
        change = false;
        for (int i=0; i<nodes; i++)
            ...
            for (int j=0; j<nodes; j++)
            {
                //while (change)
                {
                    ...
                    for (int k=0; k<nodes; k++)
                    {
                        if(k != i && k != j && i != j)
                        {
                            if (path_distances[i][j] > path_distances[i][k] + path_distances[k][j])
                            {
                                //Respect edge weight order to ensure deadlock free routing - once you are on a high-weight edge,
                                //you cannot come down to lower-weight edge
                                if(max_weight_edge[i][k] <= min_weight_edge[k][j])
                                {
                                    inter_switches[i][j] = inter_switches[i][k] + inter_switches[k][j] + 1;
                                    path_distances[i][j] = path_distances[i][k] + path_distances[k][j];
                                    max_weight_edge[i][j] = max(max_weight_edge[i][k], max_weight_edge[k][j]);
                                    min_weight_edge[i][j] = min(min_weight_edge[i][k], min_weight_edge[k][j]);
                                    change = true;
                                }
                            }
                            //else
                            //{
                            //fptr<< "Rejected path "<<i <<" -> " << k << "->" << j << " because of weight issue "
                            //<< max_weight_edge[i][k] << " "<<min_weight_edge[k][j]<<endl;
                            //}
                        }
                    }
                }
    ...
    for(int i=0;i<path_distances.size();++i)
        for(int j=0;j<path_distances[i].size();++j)
            fptr << i<< " "<< j << " "<<path_distances[i][j] << endl;

    fptr.close();
}

static void shortest_path_latency(Matrix& edge_weights, Matrix& path_distances, Matrix& inter_switches,
        Matrix& max_weight_edge, Matrix& min_weight_edge)
{
    extend_shortest_path_latency(edge_weights, path_distances, inter_switches, max_weight_edge, min_weight_edge);
}
...
static bool link_is_shortest_path_to_node_latency(SwitchID src, SwitchID next, SwitchID final, 
     const Matrix& dist, const Matrix& max_weight_edge, const Matrix& min_weight_edge)
{
  return (dist[src][next] + dist[next][final] == dist[src][final] && 
		  max_weight_edge[src][next] <= min_weight_edge[next][final]);
}

static NetDest shortest_path_to_node_latency(SwitchID src, SwitchID next,
     const Matrix& dist, const Matrix& max_weight_edge, const Matrix& min_weight_edge)
{
  NetDest result;
  int d = 0;
  ...
  for (int m=0; m<machines; m++) {
    for (int i=0; i<MachineType_base_count((MachineType)m); i++) {
      ...
      // [MachineType_base_number(MachineType_NUM)...2*MachineType_base_number(MachineType_NUM)-1]
      // for the component network
      if (link_is_shortest_path_to_node_latency(src, next, 
                                        d+max_machines,
                                        dist, max_weight_edge, min_weight_edge)) {
        MachineID mach = {(MachineType)m, i};
        result.add(mach);
      }
      d++;
