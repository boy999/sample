...
{
  //    cerr<<"VC woken up "<<m_router->get_id()<<"@"<<g_eventQueue_ptr->getTime()<<endl;
...
{
	vector<int>* outport_fa = m_input_unit[inport_iter]->get_route_fa(invc_iter);
  vector<int>* outport_df = m_input_unit[inport_iter]->get_route_df(invc_iter);
  vector<int>* output_vn_df = m_input_unit[inport_iter]->get_route_vn_df(invc_iter);

  vector<int>::iterator it;
  if(outport_fa->size() != 0) // The packet is still in the full adaptive network
  {
    int outvc_offset_iter;
    int num_vcs_per_vnet = m_vc_per_vnet;

    for(it = outport_fa->begin(); it != outport_fa->end(); ++it)
    {
      int outport = (*it);
      int vnet = (m_input_unit[inport_iter]->peekTopFlit(invc_iter))->get_vnet(); // Be careful, this vnet only means the vnet id in a virtual class
      ...
      outvc_offset_iter = 0;
      for(; outvc_offset_iter < num_vcs_per_vnet; outvc_offset_iter++)
      {
        outvc_offset++;
        if(outvc_offset >= num_vcs_per_vnet)
          outvc_offset = 0;
        int outvc = outvc_base + outvc_offset;	
        if(outport >= m_output_unit.size() || outport < 0)
          cout<<m_router->get_id()<<" "<<outport<<endl;
    ...
    if (outvc_offset_iter==num_vcs_per_vnet)
    {
      m_round_robin_invc[inport_iter][invc_iter]--;
      vector<int>::iterator itt;
      // Failed to allocate any vc in the full adaptive network
      // Thus the packet may be moved to the deadlock-free network
      for(it = outport_df->begin(), itt = output_vn_df->begin(); it != outport_df->end(); ++it, ++itt)
      {
        int outport = (*it);
        int vnet = (*itt);
    ...
    if(outvc_offset_iter==num_vcs_per_vnet)
    {
      m_router->incVCStalls();
    }
  }
  else // the packet is in the deadlock-free network
  {
    int outvc_offset_iter;
    vector<int>::iterator itt;
    int num_vcs_per_vnet = m_vc_per_vnet;
...
void VCallocator_d::arbitrate_outvcs()
{
	for(int outport_iter = 0; outport_iter < m_num_outports; outport_iter++)
	{
		for(int outvc_iter = 0; outvc_iter < m_num_vcs; outvc_iter++)
		{
			...
			// modified code to allow switching of vnets (in case of deadlock!)
			//  - Adam

			for(int in_iter = 0; in_iter < m_num_inports*m_num_vcs; in_iter++)
			{
				invc_offset++;
				if(invc_offset >= m_num_vcs)
				{
					invc_offset = 0;
					inport++;
				...
				int invc = invc_offset;
				if(m_outvc_req[outport_iter][outvc_iter][inport][invc])
				{
					m_global_arbiter_activity++;
					m_input_unit[inport]->grant_vc(invc, outport_iter, outvc_iter);
					m_output_unit[outport_iter]->update_vc(outvc_iter, inport, invc);
					#ifdef NOC_DEBUG
						flit_d *t_flit = m_input_unit[inport]->peekTopFlit(invc);
						cerr<<"VC:"<<m_router->get_id()<<" "<<outport_iter<<" I"<<t_flit->get_msg_ptr()->getIndex();
						cerr<<" from invc " << invc << " alloc outvc " << outvc_iter;
...
{
	for(int i = 0; i < NUMBER_OF_VIRTUAL_CLASS*NUMBER_OF_VIRTUAL_NETWORKS; i++)
	{
	...
	cerr<<"Could not determine vc"<<endl;
	return -1;
}
