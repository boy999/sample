...
public:
	...
	void grant_vc(int outport, int out_vc);
	void change_vc(int out_vc);//Mishali
	...
	inline vector<int>* get_route_fa() 
	{ 
		return &route_fa; 
	}
  inline vector<int>* get_route_df()
  {
    return &route_df;
  }
  inline vector<int>* get_route_vn_df()
  {
    return &route_vn_df;
  }
  inline int get_outport()
  {
    return m_outport;
  }

  inline void set_route_fa(vector<int>& outport_fa)
  {
    route_fa = outport_fa;
  }
  inline void set_route_df(vector<int>& outport_df)
  {
    route_df = outport_df;
  }
  inline void set_route_vn_df(vector<int>& output_vn_df)
  {
    route_vn_df = output_vn_df;
  }
...
private:
  ...
  vector<int> route_fa; // routing decision for full adaptive virtual network
  vector<int> route_df; // routing decision for deadlock-free virtual network
  vector<int> route_vn_df; // sub virtual network decision for deadlock-free virtual network: high or low

  int m_outport; // outport decided by VC allocator when adaptive routing is used
