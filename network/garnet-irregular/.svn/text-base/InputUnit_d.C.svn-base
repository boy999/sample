...
{
#ifdef NOC_DEBUG
  // cerr<<"    InputUnit_R:"<<m_router->get_id()<< " checking NL:" 
  //    << m_in_link->get_id() << " status: " << m_in_link->isReady() << endl ;
#endif
  flit_d *t_flit;
  // if(m_in_link->isReady())
  while(m_in_link->isReady())
  {
    t_flit = m_in_link->consumeLink();
    int vc = t_flit->get_vc();
    if((t_flit->get_type() == HEAD_) || (t_flit->get_type() == HEAD_TAIL_))
    {
      //  cerr<<"VC "<<vc<<" "<<g_eventQueue_ptr->getTime()<<endl;
      //Mishali: FIX
