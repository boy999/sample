...
void SWallocator_d::arbitrate_inports()
{
	...
	for(int inport = 0; inport < m_num_inports; inport++)
	{
      ...
      else if(m_input_unit[inport]->need_stage(invc, ACTIVE_, SA_)){ 
        //else clause added by Mishali to check for buffer full condition
        int outport = m_input_unit[inport]->get_outport(invc);
        int outvc = m_input_unit[inport]->get_outvc(invc);
        if (/*!deadlock &&*/ m_output_unit[outport]->get_credit_cnt(outvc)<=0)
        {
          GarnetNetwork_d* gnet = m_router->get_net_ptr(); 		    
          int node = m_router->get_id();
          flit_d *t_flit = m_input_unit[inport]->peekTopFlit(invc); // peeks at top flit on Input Unit
          MsgPtr msg_ptr = t_flit->get_msg_ptr();
          NetworkMessage* net_msg_ptr = dynamic_cast<NetworkMessage*>(msg_ptr.ref()); 	  
          NetDest msg_dest = net_msg_ptr->getInternalDestination();
          int dest = msg_dest.getAllDest()[0] % (MachineType_base_number(MachineType_NUM) / 3);
          bool isRfPort = m_router->getRFLinkMap(dest) == outport;
          gnet->incrementBwStall(node, isRfPort);
        }
      }
    }
  }
...
bool SWallocator_d::is_candidate_inport(int inport, int invc)
{
	...
	if((m_router->get_net_ptr())->isVNetOrdered(t_vnet))
	{
		for(int vc_offset = 0; vc_offset < m_vc_per_vnet; vc_offset++)
		{
			...
			if(m_input_unit[inport]->need_stage(temp_vc, ACTIVE_, SA_) && (m_input_unit[inport]->get_outport(temp_vc) == outport) && (m_input_unit[inport]->get_enqueue_time(temp_vc) < t_enqueue_time))
			{
				return false;		
				break;
			}
  ...
  for(int outport = 0; outport < m_num_outports; outport++)
  {
    ...
    for(int inport = 0; inport < m_num_inports; inport++)
    {
      ...
      if(m_port_req[outport][in_port]) // This Inport has a request this cycle for this port
      {
        ...
        flit_d *t_flit = m_input_unit[in_port]->peekTopFlit(invc); // peeks at top flit on Input Unit

        // if everything is ok...NOW we will remove
        // the flit from m_input_unit...
        t_flit = m_input_unit[in_port]->getTopFlit(invc); // removes flit from Input Unit
        t_flit->advance_stage(ST_);
        t_flit->set_vc(outvc);
        t_flit->set_outport(outport);
        t_flit->set_time(g_eventQueue_ptr->getTime() + 1);
#ifdef NOC_DEBUG
        cerr<<"SWA:"<<m_router->get_id()<<" INVC: "<<invc<<" OP:"<<outport<<" I"<<t_flit->get_msg_ptr()->getIndex()<<" f:"<<t_flit->get_id();
...
int SWallocator_d::get_vnet(int invc)
{
	...
	cerr<<"Could not determine vc: "<<invc<<endl;
	return -1;
}
