...
public:
    ...
    void outvcs_schedSWalloc();
    bool is_candidate_inport(int inport, int invc);
    bool isCandidateInport(int outport, int in_port);

    inline double get_success_speculation()
    {
        return m_success_speculation;
    }
    inline double get_failure_speculation()
    {
        return m_failure_speculation;
    }
#endif
...
#ifdef SPECULATIVE
    double m_success_speculation, m_failure_speculation;
#endif

    Vector<Vector<Vector<Vector<bool > > > > m_outvc_req_alloc;
	Router_d *m_router;
