...
void SWallocator_d::init()
{
    ...
    m_outport_sched.setSize(m_num_outports);
    m_inport_sched.setSize(m_num_outports);
    ...
    for(int i = 0; i < m_num_outports; i++)
    {
        ...
        m_outport_sched[i] = make_pair(g_eventQueue_ptr->getTime(), false);
        m_inport_sched[i] = make_pair(g_eventQueue_ptr->getTime(), false);
...
#ifdef SPECULATIVE
    if( m_outport_sched[outport].first == g_eventQueue_ptr->getTime()
            && m_outport_sched[outport].second == true)
        return false;

    if( m_inport_sched[inport].first == g_eventQueue_ptr->getTime()
            && m_inport_sched[inport].second == true)
        return false;
#endif
...
bool SWallocator_d::isCandidateInport(int outport, int in_port)
{
    int in_vc = m_vc_winners[outport][in_port];
...
*/
            if(m_port_req[outport][in_port]) // This Inport has a request this cycle for this port
            {
                ...
                m_outport_sched[outport].first = g_eventQueue_ptr->getTime();
                m_outport_sched[outport].second = true;
                m_inport_sched[in_port].first = g_eventQueue_ptr->getTime();
                m_inport_sched[in_port].second = true;
...
/*
                ...
                if(m_input_unit[inport]->need_stage(invc, ACTIVE_, VA_))
                {
                    m_input_unit[in_port]->setSuccessSpec(invc);
                }
#endif
*/

//#ifdef SPECULATIVE
//                flit_d *t_flit = m_input_unit[in_port]->peekTopFlit(invc);
//#else
