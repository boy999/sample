...
public:
	...
	void addInPort(NetworkLink_d *in_link, CreditLink_d *credit_link, Router_d*);
	void addOutPort(NetworkLink_d *out_link, CreditLink_d *credit_link, Router_d*);
	...
	int get_vnet(int vc);
    bool isStreamData(MsgPtr);


#ifdef CS_NOC
    int m_streaming_flit;
    int m_streaming_flit_nosupport;
    //Here only counts for CSNOC 0
    //since CSNOC 1 always successfully setup circuit path
    //Those are directly connected paths
    double m_num_setupreq;
    double m_num_freereq;
    double m_num_succ_exist_setupreq;
    double m_num_succ_new_setupreq;
    double m_num_fail_freeing_setupreq;
    double m_num_fail_conflict_setupreq;
#endif
