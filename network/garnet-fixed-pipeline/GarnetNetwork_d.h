...
public:
	...
	inline void increment_setup_flits()
	{
		m_setup_flits++;
	}
	inline void increment_free_flits()
	{
		m_free_flits++;
	}
    ...
    inline void increment_setup_network_latency(Time latency)
	{
		m_setup_network_latency += latency;
	}

    inline void increment_free_network_latency(Time latency)
	{
		m_free_network_latency += latency;
	}

    inline void increment_setup_queueing_latency(Time latency)
	{
		m_setup_queueing_latency += latency;
	}

    inline void increment_free_queueing_latency(Time latency)
	{
		m_free_queueing_latency += latency;
	}
...
//    SetupNetwork* m_setupnet_ptr; // setup network pointer
    int m_num_CSNOC;
    Vector<CircuitNetwork*> m_circuitnet_ptr; // circuit network pointer

    void printCircuitMap();
    pair<int, int> findLocation(int nodeId);
    bool CirNetHasNoConflict(int CirNetLayer, int src, int dest, bool preFreeFlag);
    bool CirNetConflictNotDetected(int CirNetLayer, int src1, int dest1, int src2, int dest2);
    void streamDataPSCount(MsgPtr, int);
    void streamDataCSCount(MsgPtr, int);
    map<string, int> m_streamData_ps;
    map<string, int> m_streamData_cs;
    double m_setup_network_latency, m_free_network_latency;
    double m_setup_queueing_latency, m_free_queueing_latency;
#endif //CS_NOC
...
/***********Data Members*************/
	...
	int m_setup_flits, m_free_flits;
