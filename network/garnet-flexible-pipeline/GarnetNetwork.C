...
Network* Network::createNetwork(int nodes)
{
    ...
    if(NetworkConfig::isGarnetNetwork() && !NetworkConfig::isIdealNetwork())
    {
    ...
    else if(NetworkConfig::isIdealNetwork())
    {
        return new IdealNetwork(nodes);
    }
    else 
...
GarnetNetwork::GarnetNetwork(GarnetNetwork_d* gnetd_ptr, int nodes)
{
    m_gnetd_ptr = gnetd_ptr;
