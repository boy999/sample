...
bb27.preheader.split:		; preds = %bb27.preheader
	...
	call i32 (...)* @loop_enter( i32 5 )		; <i32>:1 [#uses=0]
	br label %bb27.outer

bb27.outer:		; preds = %bb33, %bb27.preheader.split
	%i.0.reg2mem.0.ph = phi i32 [ 0, %bb27.preheader.split ], [ %indvar.next63, %bb33 ]		; <i32> [#uses=4]
...
bb2:		; preds = %bb19.preheader, %bb2
	%k.050 = phi i32 [ 0, %bb19.preheader ], [ %indvar.next, %bb2 ]		; <i32> [#uses=5]
	%zext5 = zext i32 %k.050 to i64		; <i64> [#uses=0]
	%zext4 = zext i32 %k.050 to i64		; <i64> [#uses=0]
	%zext2 = zext i32 %k.050 to i64		; <i64> [#uses=0]
...
bb19.preheader:		; preds = %bb.nph54.split, %bb24
	%j.052 = phi i32 [ 0, %bb.nph54.split ], [ %indvar.next57, %bb24 ]		; <i32> [#uses=4]
	%zext3 = zext i32 %j.052 to i64		; <i64> [#uses=0]
	%zext1 = zext i32 %j.052 to i64		; <i64> [#uses=0]
	%tmp = add i32 %p, -1		; <i32> [#uses=1]
	%tmp55 = icmp slt i32 %tmp, 0		; <i1> [#uses=1]
	%tmp56 = select i1 %tmp55, i32 1, i32 %p		; <i32> [#uses=1]
	call i32 (...)* @loop_enter( i32 6 )		; <i32>:3 [#uses=0]
	br label %bb2
