; ModuleID = 'omega_output.bc'
target datalayout = "e-p:64:64:64-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:64:64-f32:32:32-f64:64:64-v64:64:64-v128:128:128-a0:0:64-s0:64:64-f80:128:128"
target triple = "x86_64-unknown-linux-gnu"
	%struct.FILE = type { i32, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, %struct._IO_marker*, %struct.FILE*, i32, i32, i64, i16, i8, [1 x i8], i8*, i64, i8*, i8*, i8*, i8*, i64, i32, [20 x i8] }
	%struct._IO_marker = type { %struct._IO_marker*, %struct.FILE*, i32 }
@.str = internal constant [17 x i8] c"Now at denoise \0D\00"		; <[17 x i8]*> [#uses=1]
@.str1 = internal constant [11 x i8] c"data/T.img\00"		; <[11 x i8]*> [#uses=1]
@.str2 = internal constant [3 x i8] c"rb\00"		; <[3 x i8]*> [#uses=1]
@.str3 = internal constant [14 x i8] c"reading error\00"		; <[14 x i8]*> [#uses=1]

define void @denoise_CPU_stage1(float* %T0, float* %g, float* %u) nounwind  {
entry:
	br label %bb385.outer

bb385.outer:		; preds = %bb392, %entry
	%indvar442 = phi i32 [ 0, %entry ], [ %p.0.reg2mem.0.ph, %bb392 ]		; <i32> [#uses=1]
	%p.0.reg2mem.0.ph = add i32 %indvar442, 1		; <i32> [#uses=3]
	%tmp5 = shl i32 %p.0.reg2mem.0.ph, 8		; <i32> [#uses=1]
	br label %bb376.preheader

bb2:		; preds = %bb376.preheader, %bb2
	%indvar = phi i32 [ 0, %bb376.preheader ], [ %m.0431, %bb2 ]		; <i32> [#uses=1]
	%m.0431 = add i32 %indvar, 1		; <i32> [#uses=9]
	%tmp11 = add i32 %m.0431, %tmp9		; <i32> [#uses=1]
	...
	%tmp35 = add i32 %tmp33, %m.0431		; <i32> [#uses=1]
	...
	%tmp96 = add i32 %tmp94, %m.0431		; <i32> [#uses=1]
	...
	%tmp155 = add i32 %tmp154438, %m.0431		; <i32> [#uses=1]
	...
	%tmp213 = add i32 %tmp212, %m.0431		; <i32> [#uses=1]
	...
	%tmp272 = add i32 %tmp270, %m.0431		; <i32> [#uses=1]
	...
	%tmp332 = add i32 %tmp330, %m.0431		; <i32> [#uses=1]
	...
	%exitcond = icmp eq i32 %m.0431, 254		; <i1> [#uses=1]
	br i1 %exitcond, label %bb382, label %bb2

bb382:		; preds = %bb2
	%exitcond441 = icmp eq i32 %n.0433, 254		; <i1> [#uses=1]
	br i1 %exitcond441, label %bb392, label %bb376.preheader

bb376.preheader:		; preds = %bb382, %bb385.outer
	%indvar439 = phi i32 [ 0, %bb385.outer ], [ %n.0433, %bb382 ]		; <i32> [#uses=1]
	%n.0433 = add i32 %indvar439, 1		; <i32> [#uses=3]
	%tmp7 = add i32 %tmp5, %n.0433		; <i32> [#uses=1]
	%tmp9 = shl i32 %tmp7, 8		; <i32> [#uses=7]
	%tmp33 = add i32 %tmp9, 256		; <i32> [#uses=1]
	%tmp94 = add i32 %tmp9, -256		; <i32> [#uses=1]
	%tmp154438 = or i32 %tmp9, 1		; <i32> [#uses=1]
	%tmp212 = add i32 %tmp9, -1		; <i32> [#uses=1]
	%tmp270 = add i32 %tmp9, 65536		; <i32> [#uses=1]
	%tmp330 = add i32 %tmp9, -65536		; <i32> [#uses=1]
	br label %bb2

bb392:		; preds = %bb382
	%exitcond444 = icmp eq i32 %p.0.reg2mem.0.ph, 254		; <i1> [#uses=1]
	br i1 %exitcond444, label %return.split, label %bb385.outer

return.split:		; preds = %bb392
	ret void
}
...
entry:
	br label %bb399.outer

bb399.outer:		; preds = %bb406, %entry
	%indvar455 = phi i32 [ 0, %entry ], [ %p.0.reg2mem.0.ph, %bb406 ]		; <i32> [#uses=1]
	%p.0.reg2mem.0.ph = add i32 %indvar455, 1		; <i32> [#uses=3]
	%tmp13 = shl i32 %p.0.reg2mem.0.ph, 8		; <i32> [#uses=1]
	br label %bb390.preheader

bb10:		; preds = %bb390.preheader, %bb10
	%indvar = phi i32 [ 0, %bb390.preheader ], [ %m.0443, %bb10 ]		; <i32> [#uses=1]
	%m.0443 = add i32 %indvar, 1		; <i32> [#uses=9]
	%tmp19 = add i32 %m.0443, %tmp17		; <i32> [#uses=1]
	...
	%tmp88 = add i32 %tmp86, %m.0443		; <i32> [#uses=1]
	...
	%tmp119 = add i32 %tmp117, %m.0443		; <i32> [#uses=1]
	...
	%tmp150 = add i32 %tmp149451, %m.0443		; <i32> [#uses=1]
	...
	%tmp180 = add i32 %tmp179, %m.0443		; <i32> [#uses=1]
	...
	%tmp211 = add i32 %tmp209, %m.0443		; <i32> [#uses=1]
	...
	%tmp243 = add i32 %tmp241, %m.0443		; <i32> [#uses=1]
	...
	%exitcond = icmp eq i32 %m.0443, 254		; <i1> [#uses=1]
	br i1 %exitcond, label %bb396, label %bb10

bb396:		; preds = %bb10
	%exitcond454 = icmp eq i32 %n.0445, 254		; <i1> [#uses=1]
	br i1 %exitcond454, label %bb406, label %bb390.preheader

bb390.preheader:		; preds = %bb396, %bb399.outer
	%indvar452 = phi i32 [ 0, %bb399.outer ], [ %n.0445, %bb396 ]		; <i32> [#uses=1]
	%n.0445 = add i32 %indvar452, 1		; <i32> [#uses=3]
	%tmp15 = add i32 %tmp13, %n.0445		; <i32> [#uses=1]
	%tmp17 = shl i32 %tmp15, 8		; <i32> [#uses=7]
	%tmp86 = add i32 %tmp17, 256		; <i32> [#uses=1]
	%tmp117 = add i32 %tmp17, -256		; <i32> [#uses=1]
	%tmp149451 = or i32 %tmp17, 1		; <i32> [#uses=1]
	%tmp179 = add i32 %tmp17, -1		; <i32> [#uses=1]
	%tmp209 = add i32 %tmp17, 65536		; <i32> [#uses=1]
	%tmp241 = add i32 %tmp17, -65536		; <i32> [#uses=1]
	br label %bb10

bb406:		; preds = %bb396
	%exitcond457 = icmp eq i32 %p.0.reg2mem.0.ph, 254		; <i1> [#uses=1]
	br i1 %exitcond457, label %return.split, label %bb399.outer

return.split:		; preds = %bb406
	ret void
}

define void @denoise(float* %T0) {
entry:
	%tmp1 = tail call i32 @puts( i8* getelementptr ([17 x i8]* @.str, i32 0, i64 0) )		; <i32> [#uses=0]
	%tmp9 = tail call i8* @calloc( i64 16777216, i64 4 ) nounwind 		; <i8*> [#uses=4]
	%tmp910 = bitcast i8* %tmp9 to float*		; <float*> [#uses=6]
	%tmp2123 = bitcast float* %T0 to i8*		; <i8*> [#uses=2]
	tail call void @llvm.memcpy.i64( i8* %tmp9, i8* %tmp2123, i64 67108864, i32 4 )
	%tmp30 = tail call i8* @calloc( i64 16777216, i64 4 ) nounwind 		; <i8*> [#uses=2]
	...
	tail call void @denoise_CPU_stage1( float* %T0, float* %tmp3031, float* %tmp910 )
	tail call void @denoise_CPU_stage2( float* %T0, float* %tmp3031, float* %tmp910 )
	tail call void @llvm.memcpy.i64( i8* %tmp2123, i8* %tmp9, i64 67108864, i32 4 )
	free i8* %tmp9
	free i8* %tmp30
	ret void
}
...
entry:
	...
	%tmp8 = call i8* @calloc( i64 16777216, i64 4 ) nounwind 		; <i8*> [#uses=2]
	%tmp89 = bitcast i8* %tmp8 to float*		; <float*> [#uses=1]
	%tmp16 = call i8* @calloc( i64 16777216, i64 4 ) nounwind 		; <i8*> [#uses=2]
	%tmp1617 = bitcast i8* %tmp16 to float*		; <float*> [#uses=1]
	%tmp20 = call %struct.FILE* @fopen( i8* noalias  getelementptr ([11 x i8]* @.str1, i32 0, i64 0), i8* noalias  getelementptr ([3 x i8]* @.str2, i32 0, i64 0) )		; <%struct.FILE*> [#uses=3]
...
cond_true:		; preds = %entry
	%tmp25 = call i32 @puts( i8* getelementptr ([14 x i8]* @.str3, i32 0, i64 0) )		; <i32> [#uses=0]
	ret i32 1
...
bb27:		; preds = %bb51.preheader, %bb27
	...
	%tmp38 = shl i32 %i.093, 8		; <i32> [#uses=1]
	%tmp40 = add i32 %tmp38, %j.095		; <i32> [#uses=1]
	%tmp42 = shl i32 %tmp40, 8		; <i32> [#uses=1]
...
bb57:		; preds = %bb27
	...
	%exitcond104 = icmp eq i32 %indvar.next103, 256		; <i1> [#uses=1]
...
bb66:		; preds = %bb57
	...
	%exitcond106 = icmp eq i32 %indvar.next105, 256		; <i1> [#uses=1]
...
bb76:		; preds = %bb66
	...
	call void @llvm.memcpy.i64( i8* %tmp16, i8* %tmp8, i64 67108864, i32 4 )
