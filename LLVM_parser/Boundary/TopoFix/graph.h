...
class Graph
{
	public:
		...
		unsigned getMinLLVMNodeId() { return mMinLLVMNodeId; }
		void setMinLLVMNodeId(unsigned id) { mMinLLVMNodeId = id; }
		...
		void handleBlocksBeforePostDom();
		...
		//general
		void resetPropertyAllNodes(unsigned idx, unsigned val=0);
		void setLoopDataStructure(llvm::LoopDataStructure* X) { mLoopDataStructure = X;}

		static unsigned mCriticalityTypeMask;
	private:
		unsigned mId;
		...
		llvm::LoopDataStructure* mLoopDataStructure;
};
