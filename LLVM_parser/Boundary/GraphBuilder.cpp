...
//        mAtomPorts[FP_DIV].push_back(1);
        ...
        mIgnoreSet.clear();
        gOutputFptr.open("Id2Names_BD.log",ios::out);
...
//	bool GraphBuilder::runOnModule(Module& M, DriverGraphPass* driverPass, string dep_edge_filename, string blk_profile_name)
  ...
  //      cerr << "After blockCounters\n";
        ...
        cerr << "Finish loop labeling" << endl;
        ...
        {
            Function& F = (*I);
            if ((!(F.isDeclaration())) && (F.getName() != "loop_enter") && (F.getName()!="func_enter"))
            {
                mTotalFunctions++;
            }
        }
        ...
        for (Module::iterator I = M.begin(), E = M.end(); I != E; ++I)
        {
            ...
            if ((!(F.isDeclaration())) && (F.getName() != "loop_enter") && (F.getName()!="func_enter"))
            {
                ...
                BasicBlock::iterator II = I->begin();
                Instruction& currentI = (*II);
                //TerminatorInst* tInst = I->getTerminator();        
                //TerminatorInst* tInst = predBlk->getTerminator();

                std::vector<Value*> FargVec;
                FargVec.clear();
                FargVec.push_back(ConstantInt::get(FB_TYPE, mFunctionId, false));

                CallInst* funcFCall = new CallInst(mEnterFuncF, &(FargVec[0]), &(FargVec[1]), "", &currentI);
                //CallInst* funcFCall = new CallInst(mEnterLoopF, &(FargVec[0]), &(FargVec[1]), "", tInst);
...
//				currNode->setNumDynamicInstances(mBlkCounters[mBlkId]);
	...
	void GraphBuilder::runSecondPass(Function* F)
	{
        ...
        cerr << "runSecondPass: mFunctionId: " << mFunctionId << "; Function Name:" << F->getName() << endl;
                    ...
                    II != IE; ++II)
            {
                ...
                if(isa<CallInst>(currentI))
                {
                    ...
                    if(!NOTFOUND(mCallSite2Functions, key))
                    {
                        ...
                        {
                            cerr << j << ": calledFunctions: " << calledFunctions[j]->getName() << endl;
                        }
                        ...
                        for(unsigned j=0;j<calledFunctions.size();++j)
                        {
                                    ...
                                    mId2Node[mNodeId]->getNumDynamicInstances() > 0)
                            {
                                ...
                                cerr << "THe name of call function is : " << F->getName() << endl;
                                cerr << "THe name of callee function is : " << callee->getName() << endl;
        ...
        for (; I != E; ++I)
        {
            mBlkId2NumInst[mBlkId] = I->size();
            
            gOutputFptr << "Basic Block: Name: " << I->getName() << "; Id: " << mBlkId << "; size: " 
                << mBlkId2NumInst[mBlkId] << endl;

            calculateBlkILP(I, mBlkId);
/*
            for (BasicBlock::iterator II = I->begin(), IE = I->end();
                    II != IE; ++II)
            {
        ...
        for (int i = origBlkId ; i < mBlkId; i++)
        {
            gOutputFptr << "Basic Block Id: " << i << "; Xeon Cycles: " << mBlkId2XeonCycle[i] 
                << "; Atom Cycles: " << mBlkId2AtomCycle[i] << endl;
        }
        //mNodeId = origNodeId;
...
//        loopDataStructure.runOnFunction(F, loopInfo);
        ...
        for (; I != E; ++I)
        {
            ...
            for(unsigned i=0;i<predBlks.size();++i)
            {
                ...
                TerminatorInst* tInst = predBlks[i]->getTerminator();
                Instruction* firstInst = dyn_cast<Instruction>(I->begin());
                //Edge* e = mGraph->createEdge(predBlks[i], &(*firstInst));
                Edge* e = mGraph->createEdge(tInst, firstInst);
                //if(loopDataStructure.isBackEdge(predBlks[i], &(*I)))
                //    e->markBackEdge(true);
                assert(mCFG);
                if(mCFG->getEdge(predBlks[i], &(*I))->isBackEdge())
                   e->markBackEdge(true);
...
/*
        ...
        {
            cerr << "initPropagatedValuesVector : iteration : " << i << endl;
			nodeVec[i]->initPropagatedValuesVector(mGraph->getExternallyVisibleNodeVectorSize());
        }
        cerr << "Finish For Loop initPropagatedValuesVector\n";
*/
		nodeVec.clear();


        //Obtain the instruction level parallelism information of the loop
        LoopInfo* loopInfo = mDriverPass->getLoopInfo(F);
        LoopInfo::iterator lBegin = loopInfo->begin(), lEnd = loopInfo->end();
        for(;lBegin != lEnd;lBegin++)
            updateLoopILP(*lBegin);
...
//        cerr << "Begin propagate\n";
//		mGraph->propagate(mGraph->getExternallyVisibleNodes());

//        cerr << "Begin createExternallyVisibleSubGraph\n";
//		mGraph->createExternallyVisibleSubGraph();		

		//dump dot files
//        cerr << "Begin dump dot files\n";
...
//			assert(tokens.size() >= 2);
		...
		//	assert(!NOTFOUND(id2Function, src));
            ...
            Node* n1 = mCG->createNode(id2Value[src]->getName());
            id2Node[key] = n1;
...
/*     
        ...
        //CallInst* funcFCall = new CallInst(mEnterLoopF, &(FargVec[0]), &(FargVec[1]), "", &currentI);
        CallInst* funcFCall = new CallInst(mEnterLoopF, &(FargVec[0]), &(FargVec[1]), "", tInst);
        mIgnoreSet.insert(funcFCall);
*/
        gOutputFptr << mFunctionId << " " << F->getName() << endl;
...
*/
            ...
            argVec.clear();
            argVec.push_back(ConstantInt::get(FB_TYPE, mLoopId+mTotalFunctions, false));
            
            gOutputFptr << mLoopId+mTotalFunctions << " " << loop->getHeader()->getName() << endl;
